  ...
  return (
    <div className={`candidate-card ${isWinner ? 'winner-card' : ''}`}>
      ...
      <div className="candidate-image-wrapper">
        {candidate.image ? (
          <img src={`https://voting-app-mern-backend-2.onrender.com/${candidate.image}`} alt={candidate.name} className="candidate-image" />
        ) : (
...
const Results = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        const response = await axios.get('https://voting-app-mern-backend-2.onrender.com/api/auth/results');
